...
class Main_Scene extends Simulation {
    ...
    display(context, program_state) {
        ...
        let cam_check = camera.times(vec4(0, 0, 0, 1));
        if(cam_check[1] < 0) {
            camera = camera.times(Mat4.translation(0, (-cam_check[1]) + 1, 0));
            //camera = this.old_cam;
        }

        program_state.set_camera(camera);
        // TODO: holds previous camera position, declared in constructor
        this.old_cam = camera;
